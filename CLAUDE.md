...
- **JSON を `import` しないこと。** Vite は import された JSON をバンドルへ
  ...
  **例外を作らないこと。** デザインシステムのトークンは焼き込んでよい中身だが、
  それでも JSON にせず `src/tokens/spec.ts`（生成物）にしてある。
  「これは焼き込んでよいから」で穴を開けると、見張りが例外表になる
- **`src/tokens/spec.ts` を手で直さないこと。** 別リポジトリ（デザインシステム）が
  配信している `/tokens.json` の写しで、`npm run sync:tokens` が作り直す。
  ずれたら `npm run verify:tokens` が CI で落とす
