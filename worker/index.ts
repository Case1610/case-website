 ...
 * キャッシュの持たせ方を中身で変える。
 *
 * 画像のファイル名には**中身のハッシュ**が入っている（scripts/build-media.mjs）。
 * 中身が変われば名前が変わるので、1年持たせても古いものが残り続けない。
 *
 * 2026-09-13 まで名前に入っていたのは幅だけで、この前提が成り立っていなかった。
 * 同じ幅で焼き直すと、古い画像が1年配られ続ける状態だった（EXIF の向きを
 * 直したときに、その修正が届かないところだった）。
 *
